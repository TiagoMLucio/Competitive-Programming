...
void set_partitions(int k, int maxVal) {
    if (k == 0) {
        partitions.pb(current);
        return;    
    }
    for(int i = min(k, maxVal); i > 0; i--) {
        current.pb(i);
        set_partitions(k - i, i);
        current.pop_back();
    }
}
...
void solve()
{
    ...
    for(int i = 1; i < n; i++) {
        int count = 0, last = 0;
        while(i + count < n && count < 10 && a.at(i + count) > a.at(i - 1)) {
            int aux = a.at(i + count) - a.at(i - 1) + last;
            diff.at(count).pb(aux);
            last = aux;
            count++;
        }
    }
    ...
    for(int i = n - 2; i >= 0; i--) {
        int count = 0, last = 0;
        while(i - count >= 0 && count < 10 && a.at(i - count) > a.at(i + 1)) {
            int aux = a.at(i - count) - a.at(i + 1) + last;
            diff.at(count).pb(aux);
            last = aux;
            count++;
    ...
    for(auto p: partitions) {
        ...
        curDiff = 0;
        for(auto pi: p) {
            int diffIdx = sz(diff.at(pi - 1)) - 1 - idx.at(pi - 1);
            curDiff += diffIdx >= 0 ? diff.at(pi - 1).at(diffIdx) : 0;
            idx.at(pi - 1)++;
        }
