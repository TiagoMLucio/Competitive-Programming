...
void solve()
{
    ...
    vector<vector<int>> freqs(sz(s), vector<int>(26));
    freqs[0][s.at(0) - 'a']++;
    ...
    while(q--) {
        ...
        vector<int> freqsr = freqs[r];
        vector<int> freqsl = l > 0 ? freqs[l - 1] : vector<int>(26);

        int dif_letters = 0;

        for (int i = 0; i < 26; i++)
            if(freqsr[i] - freqsl[i] > 0)
                dif_letters++;

        ps(dif_letters > 2 ? "Yes" : "No");
    }
