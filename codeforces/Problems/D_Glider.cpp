...
void solve()
{
    ...
    int res {h + af.at(0).s - af.at(0).f}, last_h {h}, cur {res};

    for(int i = 1; i < n; i++) {
        dbg(dq, res, last_h, cur);
        while(!dq.empty() && last_h <= af.at(i).f - af.at(i - 1).s) {
            ii front = dq.front();
            cur -= front.s - front.f;
            dq.pop_front();
            if (dq.empty()) last_h = h;
            else {
                ii new_front = dq.front();
                last_h += new_front.f - front.s;
            }
        }
        if (dq.empty()) last_h = h;
        else last_h -= af.at(i).f - af.at(i - 1).s;
        dq.push_back(af.at(i));
        cur += af.at(i).s - af.at(i).f;
        res = max(res, cur);
    }
