...
{
    def(int, n);
    V<pair<str, int>> rounds(n);
    re(rounds);

    map<str, int> scores;
    for(auto r: rounds) scores[r.f] += r.s;
    
    int maxScore {0};
    for(auto score: scores) maxScore = max(maxScore, score.s);
    ...
    map<str, int> winnerScores;
    dbg(scores, maxScore, winners, winnerScores);
    for(auto r: rounds)
        for(auto w: winners)
            if(r.f == w) {
                winnerScores[w] += r.s;
                dbg(r, w);
                if (winnerScores[w] >= maxScore) {
                    ps(w);
                    return;
                }
            }
