...
void solve()
{
    ...
    bool vertical = (h <= H - (table.y2 - table.y1)), horizontal = (w <= W - (table.x2 - table.x1));
    ...
    ld deltaY = max(0LL, h - max(H - table.y2, table.y1));
    ld deltaX = max(0LL, w - max(W - table.x2, table.x1));

    ps(horizontal && vertical ? min(deltaX, deltaY) : (horizontal ? deltaX : deltaY));
}
