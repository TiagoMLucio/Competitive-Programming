...
void solve()
{
    ...
    int lastrow = (1 << (k - 1)), lastrow2s = a - (lastrow - 1), lastrowfree = lastrow - lastrow2s;

    dbg(k, lastrow, lastrow2s, lastrowfree);

    b -= lastrowfree;
    
    if (b > 0) {
        int afterlastrowfree = lastrowfree + 2 * lastrow2s;
        dbg(b, afterlastrowfree);
        k += (b - 1) / afterlastrowfree + 1; 
    }
    ps(k);
}
