...
void solve()
{
    ...
    V<V<set<int>>> aux(n, V<set<int>>(m));

    aux[0][0].insert(grid[0][0]);

    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            for(auto x: aux[i][j]) {
                if (j + 1 < m) aux[i][j + 1].insert(gcd(x, grid[i][j + 1]));
                if (i + 1 < n) aux[i + 1][j].insert(gcd(x, grid[i + 1][j]));
            }
        }
    }

    int res = 0;
    for(auto si: aux[n - 1][m - 1]) {
        res = si;
    }
