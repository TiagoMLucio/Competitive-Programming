...
void solve()
{
    ...
    std::sort(all(v), [](auto &left, auto &right) {
    ...
    for (int i = 0; i < bszise; i++)
    {
        for (int j = i; j < bszise; j++)
        {
            vi as;

            dbg(posb[v[i].s]);

            for (int k = posb[v[i].s]; k <= n - 1, v[k].s <= bs[j]; k++)
                if (v[k].s >= bs[i]) {
                        as.pb(v[k].f);
                }

            dbg(i, j, bs[i], bs[j], as);

            sort(all(as));

            dbg("sorted", as);

            int lmax = l - (bs[j] - bs[i]);
            int count = 0;
