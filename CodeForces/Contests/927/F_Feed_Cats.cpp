...
int compress(int x) {
    return distance(divs.begin(), divs.find(x));
}
...
void solve()
{
    ...
    for(int i = 1; i < m; i++) {
        if (inters[i].f > cur.s) {
            res_inters.pb(cur);
            cur = {max(inters[i].f, cur.s), inters[i].s};
        } else if (cur.f == inters[i].f) {
            cur.s = max(inters[i].s, cur.s);
        }
    }
    ...
    vi diffarray(sz(divs) + 1);

    for(auto x: inters) {
        diffarray.at(compress(x.f))++;
        diffarray.at(compress(x.s) + 1)--;
    }
    ...
    for(int i = 1; i < sz(divs); i++) {
        acc[i] += acc[i - 1] + diffarray.at(i - 1);
    }
    ...
    for (auto x: res_inters) {
        int max_cats {0};
        for(int i = compress(x.f) + 1; i <= compress(x.s); i++) 
            max_cats = max(max_cats, acc[i]);
        res += max_cats;
    }
