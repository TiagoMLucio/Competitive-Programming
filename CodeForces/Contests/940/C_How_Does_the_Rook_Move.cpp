...
void solve()
{
    ...
    for(int i = n & 1; i <= n; i+=2) {
        dbg(i, nCk(n, i, MODN), dfact[n - 1 - i], powmod(2,(n - i) / 2, MODN), mult(fact[n - 1 - i], powmod(2, (n - i) / 2, MODN)));

        if (i == n) res = (res + 1) % MODN;
        else res = (mult(nCk(n, i, MODN), mult(dfact[n - 1 - i], powmod(2, (n - i) / 2, MODN))) + res) % MODN;
