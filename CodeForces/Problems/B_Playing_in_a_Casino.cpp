...
void solve()
{
    ...
    viiii nums(m, {0, 0, 1123456, 1123456});
    ...
    for(int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++)
        {
            ...
            if (mi > get<0>(nums[j])) {
                dbg("oi");
                get<1>(nums[j]) = get<0>(nums[j]);
                get<0>(nums[j]) = mi;
            } else {
                get<1>(nums[j]) = max(get<1>(nums[j]), mi);
            }
            

            if (mi < get<3>(nums[j])) {
                get<2>(nums[j]) = get<3>(nums[j]);
                get<3>(nums[j]) = mi;
            } else {
                get<2>(nums[j]) = min(get<2>(nums[j]), mi);
            }
            dbg(nums);
        }
    }
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = n - 1, k = n - 1; k > 0; j -= 1, k-=2) 
            res += (col.at(j) - col.at(n - 1 - j)) * k;
    }    
