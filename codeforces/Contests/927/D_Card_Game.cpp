...
void solve()
{
    ...
    for (auto &suit: suits) {
        sort(all(suit.s));
        missing_pair += suit.f != trump ? sz(suit.s) % 2 : 0;
    }

    dbg(missing_pair);

    if (missing_pair > sz(suits.at(trump))) {
        ps("IMPOSSIBLE");
        return;
    }
    ...
    for(auto suit: suits) {
        if (suit.f != trump)
            for(int i = 0; i < sz(suit.s); i+=2) {
                str res {suit.s.at(i) + " "};
                res += (i == sz(suit.s) - 1 && sz(suit.s) % 2 == 1) ? suits[trump].at(suit_idx++) : suit.s.at(i + 1);
                ps(res);
            }
    }

    for(int i = suit_idx; i < sz(suits[trump]); i+=2) {
        ps(suits[trump].at(i) + ' ' +  suits[trump].at(i + 1));
    }
}
