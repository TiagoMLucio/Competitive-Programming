...
void solve()
{
    ...
    int pos = 0;
    int firstNotSorted = 0;
    int lastSorted = 0;

    for(auto c: s) {
        dbg(pos, lastSorted, firstNotSorted);
        if (c == '+') pos++;
        else if (c == '-') {
            pos--;
            lastSorted = min(pos, lastSorted);
            if (firstNotSorted > pos) firstNotSorted = 0;
        } else if (c == '1') {
            if (firstNotSorted != 0) {
                ps("NO");
                return;
            ...
            if (lastSorted == pos || pos <= 1) {
                ps("NO");
                return;
            }
            if (firstNotSorted == 0) firstNotSorted = pos;
        }
    }
