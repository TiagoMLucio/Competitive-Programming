...
int intersectX(int x1, int x2, int x3, int x4) {
    return max(0LL, min(x2, x4) - max(x1, x3));
}

int intersectsX(int x1, int x2, int x3, int x4, int x5, int x6) {
    return max(0LL, min({x2, x4, x6}) - max({x1, x3, x5}));
}

int intersectY(int y1, int y2, int y3, int y4) {
    return max(0LL, min(y2, y4) - max(y1, y3));
}

int intersectsY(int y1, int y2, int y3, int y4, int y5, int y6) {
    return max(0LL, min({y2, y4, y6}) - max({y1, y3, y5}));
}
...
void solve()
{
    ...
    Rect wb1 {intersect(w, b1)}, wb2 {intersect(w, b2)}, wb1b2 {intersect(wb1, wb2)};

    ps((w.area() > wb1.area() + wb2.area() - wb1b2.area()) ? "YES" : "NO");
}
