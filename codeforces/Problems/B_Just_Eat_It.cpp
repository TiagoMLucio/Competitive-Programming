...
void solve()
{
    ...
    vi prefsum(n + 1);
    for(int i = 1; i <= n; i++)
        prefsum.at(i) = prefsum.at(i - 1) + v.at(i - 1);

    int maxSum {0}, minPrefSum {0};

    for(int i = 1; i < n; i++) {
        maxSum = max(maxSum, prefsum.at(i) - minPrefSum);
        minPrefSum = min(minPrefSum, prefsum.at(i));
    }

    minPrefSum = prefsum.at(1);

    for(int i = 1; i < n; i++) {
        minPrefSum = min(minPrefSum, prefsum.at(i));
    }

    maxSum = max(maxSum, prefsum.at(n) - minPrefSum);

    ps(sum > maxSum ? "YES" : "NO");    
}
