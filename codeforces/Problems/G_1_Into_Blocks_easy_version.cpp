...
void solve()
{
    ...
    for(int i = 0; i < n; i++) {
        m_ints[a.at(i)] = {min(m_ints[a.at(i)].f, i), max(m_ints[a.at(i)].s, i)};
    }
    
    vii ints;

    for(auto mi: m_ints) ints.pb(mi.s);
    ...
    while(j < sz(ints)) {
        ii intk = ints[j - 1];
        if(ints[j].f > intk.s) j++;
        while(ints[j].f < intk.s && j < sz(ints)) {
            intk.s = max(intk.s, ints[j].s);
            j++;
        }
        ints2.pb(intk);
    }
    ...
    for(auto int2i: ints2) {
        map<int, int> freqs;
        for (int i = int2i.f; i <= int2i.s; i++) {
            freqs[a[i]]++;
        }
        ...
        count += int2i.s - int2i.f + 1 - maxf;
    }

    dbg(m_ints, ints, ints2);
