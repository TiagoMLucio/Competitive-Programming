...
vi lowmonts, highmonts, monts;

void pushLowMont(int cur, int pow10, int msd, int dig) {
    if (dig > 8) return;
    lowmonts.pb(cur);

    pow10 *= 10;
    for(int i = msd; i <= 8; i++) {
        pushLowMont(cur + i * pow10, pow10, i, dig + 1);
    }
}
...
signed main()
{
    ...
    // dbg(sz(lowmonts), sz(highmonts));

    for(int i = 1; i <= 9; i++) {
        monts.pb(i);
    }

    for(auto hm: highmonts) {
        int aux1 = hm;
        int digs1 = 0;
        while(aux1 > 0) {
            digs1++;
        ...
        for(auto lm: lowmonts) {
            ...
            if(digs2 != digs1) continue;

            int pow10 = 1;
            for(int j = 0; j < digs2; j++) pow10 *= 10;
