...
void solve()
{
    ...
    for(int i = 0; i < n; i++) {
        if (first == -1 && a.at(i) != -1) first = i;
        if (a.at(i) != -1) {
            last = i;
            if (prev != -1) ints.pb({prev, last});
            prev = i;
        }
    }
    ...
    for(int i = first - 1; i >= 0; i--)
        a.at(i) = ((first - 1) - i) % 2 ==  0 ? a.at(i + 1) * 2 : a.at(i + 1) / 2;
    for(int i = last + 1; i < n; i++)
        a.at(i) = (i - (last + 1)) % 2 ==  0 ? a.at(i - 1) * 2 : a.at(i - 1) / 2;
    ...
    for(auto [i, j]: ints) {
        while(i < j - 1) {
            if (a.at(i) ==  a.at(j)) {
                if (a.at(i) > 1) a.at(i + 1) = a.at(i) / 2;
                else a.at(i + 1) = 2 * a.at(i);
                i++;
        ...
        if (a.at(j) / 2 != a.at(i) && a.at(j) != a.at(i) / 2) {
            ps(-1);
            return;
        }
    }
