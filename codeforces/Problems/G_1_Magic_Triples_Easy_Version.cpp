...
void solve()
{
    ...
    for(auto x: s) {
        if (freqs[x] >= 3) res += freqs[x] * (freqs[x] - 1) * (freqs[x] - 2);
        if (x == 2) dbg(x, res, freqs[x], freqs[x * 3], freqs[x * 9]);
        for(int i = 2; i * i <= mx / x; i++)
            res += freqs[x] * freqs[i * x] * freqs[i * i * x];
    }
