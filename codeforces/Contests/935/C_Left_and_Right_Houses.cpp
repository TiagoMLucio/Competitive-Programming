...
void solve()
{
    ...
    int count0left = 0, count1right = 0;

    vii possible;

    for (auto c: s) if (c == '1') count1right++;

    if (count1right >= (n + 1) / 2)
            possible.pb({abs(n / 2 - 0), 0});

    for (int i = 1; i <= n; i++) {
        char c = s.at(i - 1);
        if (c == '1') count1right--;
        else count0left++;
        dbg(i, count1right, count0left);

        if (count0left >= (i + 1) / 2 && count1right >= (n - i + 1) / 2)
            possible.pb({min(abs(n / 2 - i), abs((n + 1) / 2 - i)), i});
    }   
