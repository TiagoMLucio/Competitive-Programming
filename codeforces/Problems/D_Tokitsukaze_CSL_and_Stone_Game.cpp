...
void solve()
{
    ...
    vi freqge2;
    for(auto freq: freqs)
        if(freq.s >= 2) freqge2.pb(freq.f);

    dbg(freqge2, freqs);

    if (sz(freqge2) == 0) {
        for(auto freq: freqs)
            if (freq.f != 0 && freqs[freq.f - 1] == 0) {
                first_possible = true;
                break;
            }
    }
    else if (sz(freqge2) == 1 && freqs[freqge2.at(0)] == 2)
        first_possible = freqge2.at(0) != 0 && freqs[freqge2.at(0) - 1] == 0;
    

    if (first_possible && sum % 2 != ((n - 1) * n / 2) % 2)
        ps("sjfnb");
    else ps("cslnb");
}
