...
void solve()
{
    ...
    int remove {0};
    int start_reach = min(max(l - k, r), r + m - 1);
    int end_reach = max(min(l + n - 1 + k, r + m - 1), 0LL);
    int start_remove = min(max(r + k, l), l + n - 1);
    int end_remove = max(min(r + m - 1 - k, l + n - 1), 0LL);
    int nums_to_remove = end_remove - start_remove + 1;

    dbg(start_reach, end_reach, start_remove, end_remove, nums_to_remove, mi);

    sort(mi.begin() + start_reach - r, mi.end() - end_reach + (r + m - 1));

    for (int i = end_reach, j = 0; i >= start_reach, j < nums_to_remove; i--) 
        if (mi[i] > 0) {remove += mi[i]; j++;};
...
signed main()
{
    ...
    // if (r > l && l + n + 1 + k >= r) {
    //     nums_to_remove = n - max(r - k - l, 0LL);
    //     nums_can_reach = min(l + n + k - r, m);
    //     sort(mi.begin(), mi.begin() + nums_can_reach);
    //     dbg(nums_to_remove, nums_can_reach, mi);
    //     for(int i = nums_can_reach - 1, j = 0; i >= 0  && j < nums_to_remove; i--)
    //         if (mi[i] > 0) {remove += mi[i]; j++;}
    // } else if (r < l && l - k <= r + m - 1) {
    //     nums_to_remove = max(r + m - 1 + k - l, n);
    //     nums_can_reach = min(r + m - l + k, m);
    //     sort(mi.end() - nums_can_reach, mi.end());
    //     for(int i = 0, j = 0; i < nums_can_reach && j < nums_to_remove; i++)
    //         if (mi[i] > 0) {remove += mi[m - 1 - i]; j++;}
    // }