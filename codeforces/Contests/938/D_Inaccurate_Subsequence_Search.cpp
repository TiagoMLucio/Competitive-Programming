...
void solve()
{
    ...
    map<int, int> freqb, freqc, missing;
    for(auto bi: b) freqb[bi]++;

    for(int i = 0; i < m; i++) freqc[a.at(i)]++;

    int res {0}, i {0}, j {m};

    int equal = 0;

    for(auto freqbi: freqb) {
        equal += min(freqc[freqbi.f], freqbi.s);
        missing[freqbi.f] += freqbi.s - freqc[freqbi.f];
    }
    ...
    while(j < n) {
        ...
        if (freqb[a.at(i)] > 0) {
            missing[a.at(i)]++;
            if (missing[a.at(i)] > 0) equal--;
        }
        if (freqb[a.at(j)] > 0) {
            missing[a.at(j)]--;
            if(missing[a.at(j)] >= 0) equal++;
        }
        i++; j++;
        if (equal >= k) res++;
    }
