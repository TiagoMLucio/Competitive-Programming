...
void solve()
{
    ...
    int sb {0}, ss {0}, kb {k}, ks {k};

    int maxb {a.at(pb)}, sumb {0}, maxs {a.at(pss)}, sums {0} , posb {pb}, poss {pss};

    do {
        posb = p.at(posb);
        maxb = max(maxb, a.at(posb));
    } while (posb != pb);

    do {
        poss = p.at(poss);
        maxs = max(maxs, a.at(poss));
    } while (poss != pss);

    dbg(maxb, maxs);

    while (kb > 0) {
        sb = max(a.at(pb) * kb + sumb, sb);
        sumb += a.at(pb);
        kb = (a.at(pb) == maxb) ? 0 : kb - 1;
        pb = p.at(pb);
    }

    while (ks > 0) {
        dbg(ss, sums, pss, ks);
        ss = max(a.at(pss) * ks + sums, ss);
        sums += a.at(pss);
        ks = (a.at(pss) == maxs) ? 0 : ks - 1;
        pss = p.at(pss);
        dbg(ss, sums, pss, ks);
    }
