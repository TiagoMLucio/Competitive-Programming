...
void solve()
{
    ...
    for(int i = 1; i < n; i++) {
        if (hup == up && hright == right) break;
        if(up != hup) {
            if (up > 0 || (up == 0 && hup < 0)) {
                if (s.at(i) == 'N') {hup++; hover.pb(i);};
            } else {
                if (s.at(i) == 'S') {hup--; hover.pb(i);};
            }
        }

        if(right != hright) {
            if (right > 0 || (right == 0 && hright < 0)) {
                if (s.at(i) == 'E') {hright++; hover.pb(i);};
            } else {
                if (s.at(i) == 'W') {hright--; hover.pb(i);};
            }
        }
    }

    dbg(up, right, hup, hright, hover);

    if(sz(hover) == n || hup != up || hright != right) {
        ps("NO");
        return;
    }
