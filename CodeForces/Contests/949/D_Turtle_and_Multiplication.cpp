...
void solve()
{
    ...
    if (n > 0) {
        for(int i = 0; i < min(n, A / 2); i++)
            res.at(2 * A + A / 2 + i) = 2 * (i + 1) - 1;
    }

    n -= A / 2;
    if (n > 0) {
        for(int i = 0; i < min(n, A / 3); i++)
            res.at(3 * A + i) = 3 * (i + 1) - 1;
    }

    map<int, int> test;

    for(int i = 0; i < m - 1; i++) {
        if (res.at(i) * res.at(i + 1) == 30) dbg(i, res.at(i), res.at(i + 1));
        test[res.at(i) * res.at(i + 1)]++;
    }

    for(auto testi: test)
        if(testi.s > 1) ps(testi);
