...
void solve()
{
    ...
    vi prefix_max(n), suffix_max(n);

    for(int i = 0; i < n; i++) {
        prefix_max.at(i) = b.at(i) + i;
        suffix_max.at(i) = b.at(i) - i;
    }
    for (int i = 1; i < n; i++)
        prefix_max.at(i) = max(prefix_max.at(i), prefix_max.at(i - 1));
    for (int i = n - 2; i >= 0; i--) 
        suffix_max.at(i) = max(suffix_max.at(i), suffix_max.at(i + 1));
    
    int res {0};

    for (int m = 1; m < n - 1; m++) 
        res = max(res, b.at(m) + prefix_max.at(m - 1) + suffix_max.at(m + 1));
