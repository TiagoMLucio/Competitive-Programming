...
int dp(int r) {
    ...
    int aux = max(val(r) + dp(r - 2), dp(r - 1));
    memo[r] = aux;
    return aux;
}
...
void solve()
{
    ...
    for(int i = 0; i < sz(vfreqs); i++) {
        dbg(i);
        if (sz(seq) == 0 || vfreqs[i].f == vfreqs[i-1].f + 1) {
            seq.pb(vfreqs[i]);
        } else {
            memo.resize(sz(seq), -1);
            res += dp(sz(seq) - 1);
            seq.clear();
            memo.clear();
            seq.pb(vfreqs[i]);
