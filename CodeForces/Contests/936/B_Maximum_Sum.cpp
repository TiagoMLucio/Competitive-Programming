...
void solve()
{
    ...
    vi minprefsum(n + 1);
    for(int i = 1; i <= n; i++) {
        minprefsum[i] = min(minprefsum[i - 1], prefsum[i]);
    }
    dbg(prefsum, minprefsum);

    int maxsum {0};

    for(int i = 1; i <= n; i++) {
        maxsum = max(maxsum, prefsum[i] - minprefsum[i]);
    }

    dbg(((pow(2, k, MODN) + MODN - 1 ) % MODN), (maxsum % MODN), ((prefsum[n] + MODN) % MODN));

    ps((mult((pow(2, k, MODN) - 1), maxsum, MODN) + modm(prefsum[n], MODN)) % MODN);
}
