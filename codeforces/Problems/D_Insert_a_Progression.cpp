...
void solve()
{
    ...
    int mx {0};
    for(auto ai: a) mx = max(mx, ai);

    int add1 {min(a.at(0) - 1, a.at(n - 1) - 1)}, addx {min(abs(a.at(0) - x), abs(a.at(n - 1) - x))};

    for(int i = 1; i < n; i++) {
        add1 = min(add1, (a.at(i - 1) - 1 + a.at(i) - 1) - abs(a.at(i) - a.at(i - 1)));
        addx = min(addx, abs(a.at(i - 1) - x) + abs(a.at(i) - x) - abs(a.at(i) - a.at(i - 1)));
    }

    dbg(sum, add1, mx, addx);
    
    ps(sum + add1 + (x > mx ? addx : 0));
}
