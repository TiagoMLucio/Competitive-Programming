...
void solve()
{
    ...
    vector<ii> amount_per_stand(m);

    for(int i = 0; i < m; i++) {
        def(int, ai, qi);

        amount_per_stand[i] = {i, min(ai, n/qi)};
    }

    dbg(amount_per_stand);

    sort(all(amount_per_stand), [](auto &left, auto &right) {
        if(left.s == right.s) return right.f < left.f;
        return left.s < right.s;
    });

    dbg(amount_per_stand);

    ps(amount_per_stand[m - 1].s == 0 ? -1 : amount_per_stand[m - 1].f + 1);    
}
