...
void solve()
{
    ...
    for(int i = 1; i < sz(s); i++) {
        dbg(s.at(i), dq, j, freqs);
        if (j > 0 && dq.at(j - 1) == s.at(i)) j--;
        else if (j < sz(dq) - 1 && dq.at(j + 1) == s.at(i)) j++;
        else if (j == 0 && freqs[s.at(i)] == 0) {
            dq.push_front(s.at(i));
            freqs[s.at(i)]++;
        } else if (j == sz(dq) - 1 && freqs[s.at(i)] == 0) {
            dq.push_back(s.at(i));
            freqs[s.at(i)]++;
            j++;
        } else {
