...
void solve()
{
    ...
    dbg(mn, mx, res, b.at(n));

    int minDiff = LONG_LONG_MAX;

    for(int i = 0; i < n; i++) {
        if ((b.at(n) <= a.at(i) && b.at(n) >= b.at(i)) || (b.at(n) >= a.at(i) && b.at(n) <= b.at(i))) {
            minDiff = 0;
            break;
        }
        minDiff = min({minDiff, abs(b.at(i) - b.at(n)), abs(a.at(i) - b.at(n))});
    }
