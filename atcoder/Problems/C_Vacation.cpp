...
void solve()
{
    ...
    V<vi> dp(n, vi(3));
    dp.at(0).at(0) = abc.at(0).at(0);
    dp.at(0).at(1) = abc.at(0).at(1);
    dp.at(0).at(2) = abc.at(0).at(2);

    for(int i = 0; i < n - 1; i++) {
        for(int c = 0; c < 3; c++) {
            dp.at(i + 1).at((c + 1) % 3) = max(dp.at(i + 1).at((c + 1) % 3), dp.at(i).at(c) + abc.at(i + 1).at((c + 1) % 3));
            dp.at(i + 1).at((c + 2) % 3) = max(dp.at(i + 1).at((c + 2) % 3), dp.at(i).at(c) + abc.at(i + 1).at((c + 2) % 3));
        }
    }    
